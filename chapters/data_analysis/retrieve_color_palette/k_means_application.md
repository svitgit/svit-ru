...
```{image} ./fig/image_representation.gif
:width: 720px
:align: center
```
...
```{image} ./fig/array_representation.gif
:width: 720px
:align: center
```
...
print(labels[:10])
```
То есть в нашем случае, например, первый пиксель соответствует кластеру с индексом `3`, третий пиксель соответствует кластеру с индексом `6`.
...
plt.show()
```
Таким образом, `k-means` при применении для изображений может быть использован для их сжатия.
Также в ряде задач в области машинного обучения бывает полезно провести такое преобразование изображения, оставив лишь наиболее важные элемент изображения.

На этом на сегодня всё.
Как и всегда, я с радостью готов помочь разобраться со сложностями, которые могли возникнуть при прсмотре сегодняшнего эпизода.
